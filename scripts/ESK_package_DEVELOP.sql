...
END esk_mgmt;
/
create or replace PACKAGE BODY esk_mgmt AS
    init BOOLEAN := FALSE;
...
select
    ...
    BEGIN
        select USER_NAME into username from APP_ADMIN.ESK_USERS where USER_ID = id_user;
        if (username = 'rpirkovic') then
            return 'UVOZ_CERTIFIKATI,UVOZ_ZZNAK,UVOZ_CLANI,CERTIFIKATI_UVOZ,UVOZ_PROIZVODI,UVOZ_KOLICINE,CERTIFIKAT_PREG_VSE,CERTIFIKAT_PREG_MOJA_ORG,CERTIFIKAT_AZURIRAJ,CERTIFIKAT_IZBRIS,CERTIFIKAT_IZVOZ,CERTIFIKAT_ZAKLJUCI,CERTIFIKAT_RAZVELJAVI,CERTIFIKAT_CLANI_IZBRISI,CERTIFIKAT_PREVERI_VELJANOST,CERTIFIKAT_OSVEZI_PARTNERJE,SIFRANTI_PREGLED,PROIZVOD_PREGLED,PROIZVOD_AZURIRAJ,ZZNAK_PREGLED,ZZNAK_AZURIRAJ,DEJAVNOST_PREGLED,DEJAVNOST_AZURIRAJ,ZAK_PODL_PREGLED,ZAK_PODL_AZURIRAJ,POIZVEDBA_PRIPRAVLJENA_POROCILA,KOLICINE_PREGLED,PRILOGE_CLAN_PREGLED,PRILOGE_PROIZVOD_PREGLED,DEJAVNOST_PREGLED,DEJAVNOST_AZURIRAJ';
        end if;
        if (username = 'astrekelj') then
           return 'UVOZ_CERTIFIKATI,UVOZ_ZZNAK,UVOZ_CLANI,UVOZ_PROIZVODI,UVOZ_KOLICINE,CERTIFIKAT_PREG_MOJA_ORG,CERTIFIKAT_AZURIRAJ,CERTIFIKAT_IZBRIS,CERTIFIKAT_IZVOZ,CERTIFIKAT_ZAKLJUCI,CERTIFIKAT_RAZVELJAVI,CERTIFIKAT_CLANI_IZBRISI,CERTIFIKAT_PREVERI_VELJANOST,CERTIFIKAT_OSVEZI_PARTNERJE,SIFRANTI_PREGLED,PROIZVOD_PREGLED,PROIZVOD_AZURIRAJ,ZZNAK_PREGLED,ZZNAK_AZURIRAJ,DEJAVNOST_PREGLED,DEJAVNOST_AZURIRAJ,ZAK_PODL_PREGLED,ZAK_PODL_AZURIRAJ,POIZVEDBA_PRIPRAVLJENA_POROCILA,KOLICINE_PREGLED,PRILOGE_CLAN_PREGLED,PRILOGE_PROIZVOD_PREGLED,DEJAVNOST_PREGLED,DEJAVNOST_AZURIRAJ';
        end if;
        if (username = 'vgrasek') then
           return 'UVOZ_ZZNAK,SIFRANTI_PREGLED,PROIZVOD_PREGLED,ZZNAK_PREGLED,ZZNAK_AZURIRAJ,DEJAVNOST_PREGLED,ZAK_PODL_PREGLED,KOLICINE_PREGLED,POIZVEDBA_PRIPRAVLJENA_POROCILA,DEJAVNOST_PREGLED,DEJAVNOST_AZURIRAJ';
        end if;
        if (username = 'jcasl') then
            return '';
        end if;
    END get_permissions;
    ...
    PROCEDURE takeoverData(id_cert IN number, v_id_user NUMBER) IS
        cnt NUMBER := 0;
    BEGIN
        DELETE FROM CERTIFIKAT_PROIZVOD WHERE 1 = 0;
    END takeoverData;
